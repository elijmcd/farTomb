...
ACMD(do_show)
{
  ...
  case 2: {
    ...
    send_to_char(ch, "Aur: %-8d  Bal: %-8d  Exp: %-8d  Align: %-5d  Lessons: %-3d\r\n",
      GET_GOLD(vict), GET_BANK_GOLD(vict), GET_EXP(vict),
      GET_ALIGNMENT(vict), GET_PRACTICES(vict));
  ...
  case 11:
    ...
    for (j = 1; j < LVL_IMMORT; j++) {
      nlen = snprintf(buf + len, sizeof(buf) - len,  "%-3d - %-2d %-2d %-2d %-2d\r\n", j, 
				thaco(CLASS_MOUNTEBANK, j),
				thaco(CLASS_AETHERIST, j),
				thaco(CLASS_DRIFTER, j),
				thaco(CLASS_OUTRIDER, j));
      if (len + nlen >= sizeof(buf))
        break;
      len += nlen;
    }
  ...
  case 12:
    ...
    for (i = 1; i < LVL_IMMORT; i++) { 
      nlen = snprintf(buf + len, sizeof(buf) - len,  "%-3d - %-6d %-6d %-6d %-6d\r\n", i,  
				level_exp(CLASS_MOUNTEBANK, i) - level_exp(CLASS_MOUNTEBANK, i - 1),
				level_exp(CLASS_AETHERIST, i) - level_exp(CLASS_AETHERIST, i - 1),
				level_exp(CLASS_DRIFTER, i) - level_exp(CLASS_DRIFTER, i - 1),
				level_exp(CLASS_OUTRIDER, i) - level_exp(CLASS_OUTRIDER, i - 1));
      if (len + nlen >= sizeof(buf))
        break;
      len += nlen;
    }
   ...
   case 21: /* int */
    ...
    case 59: /* race */
      if ((i = parse_race(*val_arg)) == RACE_UNDEFINED) {
        send_to_char(ch, "That is not a people..\r\n");
        return (0);
      }
      GET_RACE(vict) = i;
      break;
