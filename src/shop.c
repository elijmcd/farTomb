...
const char *trade_letters[] = {
        ...
        "Evil",
        "Neutral",
        "Mountebank",           /* Then the class based ones */
        "Aetherist",
        "Drifter",
        "Outrider",
        "Human",
...
static int is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr)
{
  ...
  if ((IS_HUMAN(ch) && NOTRADE_HUMAN(shop_nr)) ||
       (IS_SMALLKIN(ch) && NOTRADE_SMALLKIN(shop_nr)) ||
       (IS_WYRM(ch) && NOTRADE_WYRM(shop_nr)) ||
       (IS_UNDEAD(ch) && NOTRADE_UNDEAD(shop_nr))) {
  ...
  if ((IS_MOUNTEBANK(ch) && NOTRADE_MOUNTEBANK(shop_nr)) ||
      (IS_AETHERIST(ch) && NOTRADE_AETHERIST(shop_nr)) ||
      (IS_DRIFTER(ch) && NOTRADE_DRIFTER(shop_nr)) ||
      (IS_OUTRIDER(ch) && NOTRADE_OUTRIDER(shop_nr))) {
