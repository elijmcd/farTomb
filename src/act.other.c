...
void do_newbie(struct char_data *vict)
{
  ...
  if(GET_CLASS(vict) == CLASS_MOUNTEBANK) {
          for (i = 0; give_mountebank[i] != -1; i++) {
            obj = read_object(give_mountebank[i], VIRTUAL);
            obj_to_char(obj, vict);
          }
  }
  if (GET_CLASS(vict) == CLASS_AETHERIST) {
          for (i = 0; give_aetherist[i] != -1; i++) {
            obj = read_object(give_aetherist[i], VIRTUAL);
            obj_to_char(obj, vict);
          }
  }
  if (GET_CLASS(vict) == CLASS_DRIFTER) {
          for (i = 0; give_drifter[i] != -1; i++) {
            obj = read_object(give_drifter[i], VIRTUAL);
            obj_to_char(obj, vict);
          }
  }
  if (GET_CLASS(vict) == CLASS_OUTRIDER) {
          for (i = 0; give_outrider[i] != -1; i++) {
            obj = read_object(give_outrider[i], VIRTUAL);
            obj_to_char(obj, vict);
          }
  }
  if (GET_CLASS(vict) == CLASS_WYRM) {
          for (i = 0; give_wyrm[i] != -1; i++) {
            obj = read_object(give_wyrm[i], VIRTUAL);
            obj_to_char(obj, vict);
          }
  }
  if (GET_CLASS(vict) == CLASS_UNDEAD) {
          for (i = 0; give_undead[i] != -1; i++) {
            obj = read_object(give_undead[i], VIRTUAL);
            obj_to_char(obj, vict);
          }
  }
}
