...
#define IS_AETHERIST(ch)		(!IS_NPC(ch) && \
        (GET_CLASS(ch) == CLASS_AETHERIST))
/** 1 if ch is DRIFTER class, 0 if not. */
#define IS_DRIFTER(ch)		(!IS_NPC(ch) && \
        (GET_CLASS(ch) == CLASS_DRIFTER))
/** 1 if ch is OUTRIDER class, 0 if not. */
#define IS_OUTRIDER(ch)		(!IS_NPC(ch) && \
        (GET_CLASS(ch) == CLASS_OUTRIDER))

/* 1 if ch is human race, 0 if not */
#define IS_HUMAN(ch)            (!IS_NPC(ch) && \
        (GET_RACE(ch) == RACE_HUMAN))
/* 1 if ch is elf race, 0 if not */
#define IS_SMALLKIN(ch)            (!IS_NPC(ch) && \
        (GET_RACE(ch) == RACE_SMALLKIN))
/* 1 if ch is elf race, 0 if not */
#define IS_WYRM(ch)            (!IS_NPC(ch) && \
        (GET_RACE(ch) == RACE_WYRM))
